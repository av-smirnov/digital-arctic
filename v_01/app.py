        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(id='country_page_indicator_dropdown',
                             ...
                             value='Численность населения, человек',
                             options=[{'label': indicator, 'value': indicator}
                                     for indicator in stats.columns[3:12]]),                 
            ], lg=6, md=11),
            dbc.Col([
...
def plot_country_charts(pathname, areas, indicator):
    ...
    if unquote(pathname[1:]) in areas:
        area = unquote(pathname[1:])
    df = stats[stats['Территория'].isin(areas)]
    fig = px.line(df,
                  x='Год',
                  y=indicator,
                  ...
                  color='Территория')
    fig.layout.paper_bgcolor = '#E5ECF6'
    table1 = profiles[profiles['Территория'] == areas[0]].iloc[:, 8:31]
    table2 = profiles[profiles['Территория'] == areas[0]].iloc[:, 31:54]
    table3 = profiles.iloc[0:1, 8:31]
